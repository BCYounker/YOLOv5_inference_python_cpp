...
{
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " [cuda/no-cuda] [process_number]" << std::endl;
        return -1;
    }
    ...
    bool is_cuda = strcmp(argv[1], "cuda") == 0;
    int process_number = std::stoi(argv[2]);

    std::string target_folder = "../../coco/val2017/";
    std::vector<std::string> file_names;

    // Load class list and net
    cv::dnn::Net net;
    ...
    DIR *dir;
    struct dirent *ent;
    if ((dir = opendir(target_folder.c_str())) != NULL) {
        ...
        std::cerr << "Could not open directory: " << target_folder << std::endl;
        return -1;
    }
    ...
    for (int count = 0; count < std::min(process_number, static_cast<int>(file_names.size())); ++count) {
        std::string file_name = file_names[count];
        std::string file_path = target_folder + file_name;
        ...
        std::string output_file = "../../coco/coco_output_cpp/" + file_name;
        if (!cv::imwrite(output_file, frame)) {
            std::cerr << "Error saving the image: " << output_file << std::endl;
        }
    }
